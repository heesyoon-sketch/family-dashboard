-- Migration 074: Allow a task to appear in both morning and afternoon/evening windows.
...
begin
  for v_constraint in
    ...
    where conrelid = 'public.tasks'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike '%time_window%'
  loop
    execute format('alter table public.tasks drop constraint %I', v_constraint.conname);
  end loop;
end;
$$;
...
alter table public.tasks
  ...
  check (time_window in ('morning', 'evening', 'both'));

create or replace function public.task_effective_window(
  p_task_window text,
...
as $$
  select case
    when p_task_window = 'both' then case when p_current_window = 'morning' then 'morning' else 'evening' end
    when p_task_window = 'morning' then 'morning'
    else 'evening'
  end;
$$;
...
as $$
  select case
    ...
    else p_day_start + interval '13 hours'
  end;
$$;
...
as $$
  select case
    when public.task_effective_window(p_task_window, p_current_window) = 'morning' then p_day_start + interval '13 hours'
...
as $$
  ...
  and (
    public.task_effective_window(p_time_window, p_current_window)
    =
    case when p_current_window = 'morning' then 'morning' else 'evening' end
  );
$$;
...
begin
  ...
  where tc.user_id = p_user_id
    and tc.completed_at >= public.task_completion_window_start(p_day_start, dt.time_window, p_time_window)
    and tc.completed_at < public.task_completion_window_end(p_day_start, dt.time_window, p_time_window);
...
grant execute on function public.task_effective_window(text, text) to authenticated;
grant execute on function public.task_completion_window_start(timestamptz, text, text) to authenticated;
grant execute on function public.task_completion_window_end(timestamptz, text, text) to authenticated;
