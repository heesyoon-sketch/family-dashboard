-- Migration 078: remove the legacy "+10 perfect-day bonus" from
-- process_task_completion_atomic.
--
-- The pre-redesign RPC silently added a flat +10 points to whichever
-- completion finished the day's last due task. With small-base habits
-- this turned a 10pt task into a 20pt reward (effectively +100%) — not
-- visible to the user, contradicting the "Points stay simple" principle,
-- and double-counting with the new Momentum/Harmony rhythm rewards.
--
-- This migration replaces the function with an identical implementation
-- minus the perfect-day block (and the v_active_count / v_today_count
-- counters that supported it). All other behaviour — partial-credit
-- halving, streak tracking, level/balance updates — is preserved.
--
...
begin
  ...
  -- NOTE: the legacy "+10 perfect-day bonus" block was removed here.
  -- Daily/full-completion satisfaction is communicated via the
  -- client-side Momentum and Harmony systems instead.
