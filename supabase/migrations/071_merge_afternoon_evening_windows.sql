-- Migration 071: Collapse task windows back to morning and afternoon/evening.
--
-- Afternoon and evening now share one dashboard window from 12:00-23:59.
-- Existing all-day tasks are folded into the later window so the morning
-- dashboard only shows morning routines.

update public.tasks
set time_window = 'evening'
where time_window is null
   or time_window = 'afternoon';
...
alter table public.tasks
  ...
  check (time_window in ('morning', 'evening'));
...
as $$
  ...
  and (
    case when p_time_window = 'morning' then 'morning' else 'evening' end
    =
...
begin
  ...
  v_completion_window_start := case
    when v_task.time_window = 'morning' then p_day_start
...
begin
  ...
  where tc.user_id = p_user_id
    and tc.completed_at >= case
      when dt.time_window = 'morning' then p_day_start
      else p_day_start + interval '12 hours'
    end
    and tc.completed_at < case
