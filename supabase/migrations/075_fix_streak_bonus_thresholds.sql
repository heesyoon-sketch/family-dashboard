-- Migration 075: Align streak bonus thresholds with the product rule:
-- 3+ day streak = 1.2x, 7+ day streak = 1.5x.
...
immutable
as $$
  select round(coalesce(p_points, 0) * public.streak_bonus_multiplier(p_streak))::integer;
$$;
...
grant execute on function public.streak_bonus_multiplier(integer) to authenticated;
grant execute on function public.apply_streak_bonus(integer, integer) to authenticated;
