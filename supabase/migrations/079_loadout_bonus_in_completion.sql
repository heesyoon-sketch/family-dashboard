-- Migration 079: apply the equipped-insignia + momentum + harmony bonus to
-- per-completion points inside the RPC.
--
-- Up to migration 078 the "+X% bonus" displayed on the Insignia Wall was
-- purely cosmetic — points_awarded was always exactly base_points. This
-- migration adds a `p_bonus_percent` parameter to
-- process_task_completion_atomic. The client computes the composed bonus
-- (loadout + momentum + harmony, capped on the client side) and passes it
-- in; the server applies it after a hard clamp to [0, 50] so a runaway or
-- manipulated client can never inflate points beyond 1.5×.
--
-- Backwards compatibility: callers that don't pass the new parameter still
-- work — the default of 0 means no bonus, matching pre-079 behaviour. The
-- old 7-arg signature is dropped to avoid PostgREST resolving stale calls
-- to it after the schema reload.
--
...
begin
  ...
  -- and is supposed to clamp to 50 itself, but we never trust the inbound
  -- value: 1.5× is the absolute design ceiling.
  v_bonus_pct := greatest(0::numeric, least(50::numeric, coalesce(p_bonus_percent, 0)));
  v_points := round(v_base_points * (1 + v_bonus_pct / 100.0))::integer;

  -- apply_streak_bonus is a no-op after migration 077 (always returns
  -- p_points unchanged). Kept in the call chain for backwards compat.
